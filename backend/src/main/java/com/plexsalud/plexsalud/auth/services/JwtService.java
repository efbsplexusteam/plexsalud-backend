...
public class JwtService {
    ...
    // Importa la clave secreta y el tiempo de expiración del JWT desde las propiedades de configuración
    ...
    // Extrae cualquier claim específico del token JWT utilizando una función proporcionada
    public <T> T extractClaim(String token, Function<Claims, T> claimsResolver) {
        final Claims claims = extractAllClaims(token);
        return claimsResolver.apply(claims);
    }

    // Genera un token JWT con los detalles del usuario y el tiempo de expiración predeterminado
    public String generateToken(UserDetails userDetails) {
        return generateToken(new HashMap<>(), userDetails);
    }

    // Genera un token JWT con los detalles del usuario y cualquier claim adicional especificados
    ...
    // Construye un token JWT con los claims adicionales, detalles del usuario y tiempo de expiración especificados
    private String buildToken(
        ...
        return Jwts
                ...
                .setIssuedAt(new Date(System.currentTimeMillis())) // Establece la fecha y hora actual como la fecha de emisión
                .setExpiration(new Date(System.currentTimeMillis() + expiration)) // Establece la fecha y hora de expiración
                .signWith(getSignInKey(), SignatureAlgorithm.HS256) // Firma el token con una clave secreta utilizando algoritmo HS256
